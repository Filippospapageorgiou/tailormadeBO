...
<AlertDialog.Root bind:open onOpenChange={handleOpenChange}>
  ...
  <AlertDialog.Portal>
    ...
    >
      ...
      <form 
          ...
          use:enhance={() => {
              ...
              return async ({ result }) => {
                  // Μετά την υποβολή
                  try {
                      // Περίμενε λίγο και μετά ξεκίνα το progress bar
                      await wait(200);
                      progressStore.startProgress(`Διαγραφή άρθρου: ${blogTitle}...`);
                      
                      // Περίμενε λίγο για να δείξεις την πρόοδο
                      await wait(800);
                      
                      // Ολοκλήρωσε την πρόοδο
                      progressStore.completeProgress(800);
                      
                      // Περίμενε να ολοκληρωθεί το animation του progress bar
                      await wait(1000);
                      
                      // Επιλογή ενημέρωσης UI
                      if (onSuccess) {
                          // Χρήση του callback αν έχει παρασχεθεί
                          onSuccess();
                      } else {
                          // Διαφορετικά, ανανέωση των δεδομένων με invalidateAll
                          await invalidateAll();
                      }
      ...
      >
        <input type="hidden" name="id" value={blogId.toString()} />
